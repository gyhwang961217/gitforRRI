...
# 관측소 이름을 메모장에서 읽어오기
dtest <- read.table("C:/Users/EKR/Desktop/해수 부록/staion_길상.txt", header = T, fileEncoding = "euc-kr")
STA1 <- dtest$STAID
...
view(data)
  ...
  rain <- subset(aosrain, aosrain$지점명 == rain_stations)
  ...
  # 주축 (지하수위) 그래프 그리기
  graph_dir <- "C:/Users/EKR/Desktop/graph/SEAWATER/해수/"
  dir.create(graph_dir, showWarnings = FALSE)  # 그래프 저장 폴더 생성 (없을 경우)
  
  # 그래프 파일 이름 설정
  graph1_path <- file.path(graph_dir, "도산1_graph1.png")
  graph2_path <- file.path(graph_dir, "도산1_graph2.png")
  graph3_path <- file.path(graph_dir, "도산1_graph3.png")
  combined_path <- file.path(graph_dir, "도산1_combined.png")
  ...
  # EC1과 EC2의 최소, 최대값을 함께 고려하여 범위 설정
  ymin <- min(c(EC1, EC2), na.rm=TRUE) - 400
  ymax <- max(c(EC1, EC2), na.rm=TRUE) + 400 

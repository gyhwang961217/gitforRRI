...
setwd("C:/Users/EKR/Desktop/해수 부록/" )
getwd()
#첫 값이 0이거나 이상하면 전체적으로 오류가 날 수 있음. 확인이 필요함
#그러기 위해서, 이동평균을 계산할 필요가 있음.
# 관측소 이름을 메모장에서 읽어오기
dtest <- read.table("C:/Users/EKR/Desktop/★_평가보고서 작성/staion_해수2.txt", header = T, fileEncoding = "euc-kr") #원래는2
STA1 <- dtest$STAID
...
for (i in seq_along(STA1)) { # STA1의 길이에 따라 반복
  ...
  # 관측소에 맞는 파일을 불러오기
  file_path <- paste("C:/Users/EKR/Desktop/★_평가보고서 작성/data_resource_해수/", STA1[i], ".xlsx", sep = "")  
  data <- read_xlsx(file_path, skip = 1) #쓸데없는거 삭제(skip)
  #하부 전기전도도값 없어서 안뜬다면, 엑 셀에서 첫 행 값을 그냥 0으로로
  
  #특정 데이터만 선택----
  # 관측소 데이터 불러오기
  data <- data %>%
    dplyr::select(1, 2, 3, 4, 6, 7, 8, 9)  # 필요한 열만 선택
  ...
  data <- data %>%
    mutate(
      ...
      `수온(하부)` = as.numeric(`수온(하부)`),
      `전기전도도(상부)` = as.numeric(`전기전도도(상부)`),
      `전기전도도(하부)` = as.numeric(`전기전도도(하부)`)
    ) %>%
    filter(
      `수온(상부)` > 5 & `수온(상부)` <= 25,
      `전기전도도(상부)` >= 10 & `전기전도도(상부)` <= 80000,
      `전기전도도(하부)` >= 10 & `전기전도도(하부)` <= 80000
    )
  ...
  aosrain <- read_excel("C:/Users/EKR/Desktop/★_평가보고서 작성/강수량_2024까지최종.xlsx", col_names = c("지점", "지점명", "날짜", "강수량"), skip = 1)
  aosrain <- aosrain %>%
    mutate(날짜 = ymd(날짜))
  ...
  # 주축 (지하수위) 그래프 그리기
  graph_dir <- "C:/Users/EKR/Desktop/graph/SEAWATER/전체_EC기준2/"
  dir.create(graph_dir, showWarnings = FALSE)  # 그래프 저장 폴더 생성 (없을 경우)
  
  # 그래프 파일 이름 설정
  graph1_path <- file.path(graph_dir, paste0(STA1[i], "_graph1.png"))
  graph2_path <- file.path(graph_dir, paste0(STA1[i], "_graph2.png"))
  graph3_path <- file.path(graph_dir, paste0(STA1[i], "_graph3.png"))
...
#  a<- min(dates) 
  ...
  date_range <- as.numeric(difftime(b, a, units = "days"))
  
  if (date_range <= 365 * 2) {
    interval <- "3 month"
  } else if (date_range <= 365 * 6) {
    interval <- "1 years"
  ...
  axis.Date(1, at = seq(from = year_start, to = year_end, by =  interval), format = "%y/%m/%d")
  abline(v = seq(from = year_start, to = b, by =  interval), col="black", lty=1)
  ...
  Q1 <- quantile(EC1, 0.25, na.rm=TRUE)  # 1사분위
  Q3 <- quantile(EC1, 0.75, na.rm=TRUE)  # 3사분위
  IQR_val <- Q3 - Q1

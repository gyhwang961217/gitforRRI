...
# 관측소 이름을 메모장에서 읽어오기
dtest <- read.table("staion_해수4.txt", header = T, fileEncoding = "euc-kr")
STA1 <- dtest$관정명
...
for (i in seq_along(STA1)) {  # STA1의 길이에 따라 반복
  ...
  data <- data[, c(1, 2, 3, 4, 6, 7, 8, 9)]  # 필요한 열만 선택
  ...
  data <- data %>%
    ...
    filter(
      `수온(상부)` > 3 & `수온(상부)` <= 25,
      #`수온(하부)` > 3 & `수온(하부)` <= 25,
      `전기전도도(상부)` >= 30 & `전기전도도(상부)` <= 80000,
    #  `전기전도도(하부)` >= 30 & `전기전도도(하부)` <= 80000,
      `수위(지표면기준)` > 0
    )
  ...
  chadata4 <- data %>%
    ...
    mutate(
      keep = {
        if (all(is.na(`수위(해수면기준)`)) || length(`수위(해수면기준)`) == 0) {
          rep(FALSE, n())
        } else {
          ...
          sapply(seq_along(`수위(해수면기준)`), function(i) {
            ...
            if (is.na(ref) || abs(current - ref) <= 3) {
              ref <<- current  # ref를 갱신
  ...
  write_xlsx(df1, paste0("C:/Users/EKR/Desktop/★_평가보고서 작성/해수_장기관측 추세분석/",STA1[i], ".xlsx"))
}
...
view(final_result)
# 결과를 엑셀 파일로 저장
write_xlsx(final_result, "전체_단계구분_결과_해수2.xlsx")

...
#한 번 더 제거----
chadata444 <- chadata44 %>%
  arrange(날짜) %>%  # 날짜를 기준으로 정렬
  mutate(diff = abs(`수온(상부)` - lag(`수온(상부)`))) %>%  # 바로 전 수위값과의 차이 계산
  filter(is.na(diff) | diff < 0.2)  # 차이가 0.2 미만인 데이터만 남기기

nrow(chadata444)-nrow(chadata44)

chadata4444 <- chadata444 %>%
  arrange(날짜) %>%  # 날짜를 기준으로 정렬
  mutate(diff = abs(`수온(상부)` - lag(`수온(상부)`))) %>%  # 바로 전 수위값과의 차이 계산
  filter(is.na(diff) | diff < 0.2)  # 차이가 0.2 미만인 데이터만 남기기

nrow(chadata4444)-nrow(chadata444)
view(chadata4444)
data44 <- data4 %>%
  left_join(chadata4444 %>% dplyr::select(날짜,`수온(상부)` ) %>%
              rename(tmp =`수온(상부)` ), by = "날짜")


#연속적으로 이상한 값 나오는 애들 제거함(계절성 제거로 해결)----
data4_a <- chadata4444 %>%
  time_decompose(, method = "stl", frequency = "auto") %>%
  anomalize(remainder, method = "iqr") %>%
  time_recompose() %>%
  filter(anomaly == "No") # 이상치가 아닌 값만 필터링
